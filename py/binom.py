...
class Allgemein(Slide):
    def construct(self):
        ...
        rectac = Rectangle(width=a, height=c, color=col_ac, **common_kwargs).shift(
            acshift
        )
        ...
        rectad = Rectangle(width=a, height=d, color=col_ad, **common_kwargs).shift(
            adshift
        )

        bdshift = d / 2 * DOWN + b / 2 * RIGHT + globalshift
        rectbd = Rectangle(width=b, height=d, color=col_bd, **common_kwargs).shift(
            bdshift
        )

        rect_calc = Rectangle(width=a + b, height=c + d, color=WHITE, **common_kwargs)
        ...
        braces = [brace_a, brace_b, brace_c, brace_d, atext, btext, ctext, dtext]
        ...
        self.next_slide()
        # dissection
        self.play(textrc.animate.shift((a + dist) * LEFT + c * UP))
        # get the rectangles/squares
        self.next_slide()
        self.play(Create(rectac), Transform(tac, textac))
        self.next_slide()
        self.play(Create(rectad), Transform(tad, textad))
        self.next_slide()
        self.play(Create(rectbc), Transform(tbc, textbc))
        self.next_slide()
        self.play(Create(rectbd), Transform(tbd, textbd))
...
class BinomEins(Slide):
    def construct(self):
        ...
        squarea = Square(a, color=colour_aa, **common_kwargs).shift(ashift)

        bshift = b / 2 * RIGHT + b / 2 * DOWN + globalshift
        squareb = Square(b, color=colour_bb, **common_kwargs).shift(bshift)

        abshift = b / 2 * DOWN + a / 2 * LEFT + globalshift
        rectab = Rectangle(width=a, height=b, color=colour_ab, **common_kwargs).shift(
            abshift
        )

        bashift = a / 2 * UP + b / 2 * RIGHT + globalshift
        rectba = Rectangle(width=b, height=a, color=colour_ab, **common_kwargs).shift(
            bashift
        )

        rect_calc = Square(a + b, color=colour_rc, **common_kwargs)
        ...
        # Animation
        # to be calculated
        self.play(Create(rc), *[Create(b) for b in braces0])
        self.next_slide()
        # dissection
        self.play(
        ...
        # get the rectangles/squares
        self.next_slide()
        self.play(Create(squarea), Transform(ta, texta))
        self.next_slide()
        self.play(Create(squareb), Transform(tb, textb))
        self.next_slide()
        self.play(Create(rectab), Transform(tab, textab))
        self.play(Create(rectba), Transform(tba, textba))
        self.play(
            FadeOut(rect_calc), Transform(tba, MathTex("ab").shift(tba.get_center()))
        )
        ...
        # split rectangles
        self.play(
            bb.animate.shift(RIGHT + DOWN - globalshift),
...
class BinomZwei(Slide):
    def construct(self):
        ...
        rcshift = b / 2 * (UP + LEFT)
        rect_calc = Square(a - b, color=RED, **common_kwargs).shift(rcshift)
        ...
        brace_upab = Brace(
            Line().scale((a - b) / 2).shift(a / 2 * UP + b / 2 * LEFT), direction=UP
        )
        buabtext = brace_upab.get_tex("a-b")
        ...
        self.play(
            aa.animate.shift((a + dist) * LEFT - aa.get_center() + globalshift),
            ab.animate.shift(-ab.get_center() + globalshift),
            ba.animate.shift(-ba.get_center() + globalshift),
            bb.animate.shift(
                (b / 2 + a / 2 + dist) * RIGHT - bb.get_center() + globalshift
            ),
        )
        self.play(
            FadeOut(tab), Transform(tba, MathTex("-2\cdot ab").shift(globalshift))
        )
        # text for equation
...
class BinomDrei(Slide):
    def construct(self):
        ...
        self.play(ab.animate.scale(0), ba.animate.scale(0))
        self.play(FadeOut(ab), FadeOut(ba))
        # text for equation
        gleich = MathTex("=").shift(3 * (a + dist) / 2 * LEFT + globalshift)
