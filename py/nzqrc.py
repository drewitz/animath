...
class Numbersets(Scene):
    ...
    def appearN(self):
        self.nl = NumberLine(x_range=[0, self.nmax, 1],
    ...
    def appearQ(self):
        ...
        for k in range(2, self.levelQ + 1):
            ...
            newlabels = [MathTex(r"\cdots").shift(labeldownshift + self.nmax * LEFT)] + \
                        [MathTex(
                            fr"\frac{{{i}}}{{{k}}}", color=colQ
                        ).shift(
                            labeldownshift + i * RIGHT
                        )
                            for i in indices if abs(i) < self.nmax
                        ] + \
                        [MathTex(r"\cdots").shift(labeldownshift + self.nmax * RIGHT)]
            heightshift = (newlabels[1].height / 2 + 0.1) * DOWN
            newlines = [DashedLine(i / k * RIGHT, heightshift + labeldownshift + i * RIGHT)
                        for i in indices if abs(i) < self.nmax
                        ]
            if old is None:
    ...
    def appearR(self):
        ...
        # to pi
        for x in [(np.pi, r"\pi"), (-np.e, r"-\mathrm{e}"),
                  (np.sqrt(2), r"\sqrt{2}"), (0.5*(-np.sqrt(5) + 1), r"1-\varphi")]:
            next_point(*x)
            self.wait(supershort)
    ...
    def appearC(self):
        ...
        def update_label(mobj):
            mobj.set_value(rf"{self.dotC.get_center()[0]}+{self.dotC.get_center()[1]}\cdot\mathrm{{i}}")
    ...
    def construct(self):
        ...
        self.textQ = Tex(r"$\subset \mathbb{Q}$", color=colQ).next_to(
            self.textZ, RIGHT
        )
        self.textR = Tex(r"$\subset \mathbb{R}$", color=colR).next_to(
            self.textQ, RIGHT
        )
        self.textall = VGroup(self.textN, self.textZ, self.textQ, self.textR)
        self.textall.shift((self.textN.width - self.textall.width) / 2 * RIGHT +
