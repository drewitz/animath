...
class Sine(Slide):
    def construct(self):
        ...
        ax2 = Axes(
            x_range=[0, n * 90 * DEGREES, 90 * DEGREES],
            y_range=[-axmax, axmax],
            x_length=8,
            y_length=2 * axmax,
        ...
        op = Line(
            ...
            lambda x: x.become(
                Line(
                    ax1.coords_to_point(0, 0),
                    ax1.coords_to_point(
                        np.cos(phi.get_value()), np.sin(phi.get_value())
                    ),
                )
            )
        )
        angle = Arc(
            radius=0.2, angle=phi.get_value(), arc_center=ax1.coords_to_point(0, 0)
        ).add_updater(
            lambda x: x.become(
                Arc(
                    radius=0.2,
                    angle=phi.get_value(),
                    arc_center=ax1.coords_to_point(0, 0),
                )
            )
        )
        ...
        # right part
        sine_line = ax2.plot(
            lambda x: np.sin(x), x_range=[0, phi.get_value()], color=colourmain
        )
        ...
        self.next_slide()
        self.play(
            phi.animate.set_value(150 * DEGREES),
            FadeOut(triangle),
            FadeIn(op),
            run_time=3,
        )
